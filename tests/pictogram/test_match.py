...
def test_match_returns_refs_for_known_concepts():
    ...
    assert "openmoji/apply.svg" in glyphs
    assert "mulberry/document.svg" in glyphs
    assert "openmoji/mail.svg" in glyphs
...
def test_match_deduplicates_synonyms_by_glyph():
    ...
    money = [r for r in refs if r.glyph_id == "openmoji/money.svg"]
    assert len(money) == 1
...
def test_match_preserves_first_seen_order():
    refs = match("전화 신청")
    glyphs = [r.glyph_id for r in refs]
    assert glyphs.index("mulberry/phone.svg") < glyphs.index("openmoji/apply.svg")
